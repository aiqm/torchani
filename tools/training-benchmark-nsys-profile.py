...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dry-run", help="just run it in a CI without GPU", action="store_true"
    )
    ...
    for epoch in range(0, WARM_UP_BATCHES + 1):
        ...
        for i, properties in enumerate(dataset):
            ...
            PROFILING_STARTED = not args.dry_run and (
                total_batch_counter >= WARM_UP_BATCHES
            )
            ...
            with torch.autograd.profiler.emit_nvtx(
                enabled=PROFILING_STARTED, record_shapes=True
            ):
