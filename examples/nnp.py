...
def step(energies, species, coordinates, batch_size, training, epoch, do_print, fileindex):
    ...
    if do_print:
        print('[epoch={},file={}] rmse:'.format(
            epoch, fileindex), rmse_kcalmol.item(), 'kcal/mol')
    if training:
...
def visit_file(filename, training, epoch):
    ...
    for data in adl:
        global_steps += 1
        remaining_coordinates = torch.from_numpy(
            data['coordinates']).type(aev.dtype)
        ...
        while remaining_conformations >= 2 * batch_size:
            ...
            step(energies, species, coordinates, batch_size,
                 training, epoch, do_print, fileindex)

        if remaining_conformations > 0:
            step(remaining_energies, species, remaining_coordinates,
                 remaining_conformations, training, epoch, do_print, fileindex)
