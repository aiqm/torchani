...
def build_parser():
    ...
    main_parser = argparse.ArgumentParser(
        prog="torchani", description="TorchANI Command Line Interface"
    )
    subparsers = main_parser.add_subparsers(
        dest="workflow", help="select workflow", required=True
    )
    ...
    parser_download = subparsers.add_parser(
        ...
        description=(
            "Download datasets from roitberg-group server\n."
            "Requires access to UF's network.\n"
            "Avaiable datasets are described in:\n"
            f"{REPO_BASE_URL}/blob/master/torchani/datasets/builtin.py"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser_download.add_argument(
        "dataset", type=str, choices=_BUILTIN_DATASETS, help="the dataset to download"
    )
    parser_download.add_argument(
        "lot", type=str, choices=_BUILTIN_DATASETS_LOT, help="level of theory"
    )
    parser_download.add_argument(
        "--root",
        type=str,
        default=None,
        help="Optional root dir to save the dataset, default is datasets/{name}-{lot}",
    )

    # dataset download parser
    parser_h5info = subparsers.add_parser(
        "h5info",
        help="show h5file informations",
        description="Show h5file informations.",
    )
    parser_h5info.add_argument(
        "path", type=str, help="path to a h5 dataset file or a directory"
    )
    ...
    parser_h5pack.add_argument("path", type=str, help="Path to a h5 dataset directory")
