...
return_radial_input = False
return_angular_input = False
return_computeaev_input = False
...
class MyAutograd(object):
    def save_for_backward(self, *tensors):
        self.saved_tensors = tensors


class Cutoff_cosine(torch.autograd.Function):
    @staticmethod
    def forward(ctx, distances: Tensor, cutoff: float) -> Tensor:
    ...
    def backward(ctx, grad_output):
        ...
        cutoff = ctx.constant
        return grad_output * (-0.5 * (math.pi / cutoff) * torch.sin(distances * (math.pi / cutoff))), None


class MyCutoff_cosine(MyAutograd):
    # @snoop
    def forward(self, distances: Tensor, cutoff: float) -> Tensor:
        self.save_for_backward(distances)
        self.constant = cutoff
        return 0.5 * torch.cos(distances * (math.pi / cutoff)) + 0.5

    # @snoop
    def backward(self, grad_output):
        distances, = self.saved_tensors
        cutoff = self.constant
        return grad_output * (-0.5 * (math.pi / cutoff) * torch.sin(distances * (math.pi / cutoff))), None
...
class MyRadial_terms(MyAutograd):
    # @snoop
    def forward(self, Rcr, EtaR, ShfR, distances):
        distances = distances.view(-1, 1, 1)
        fc = 0.5 * torch.cos(distances * (math.pi / Rcr)) + 0.5
        rad = 0.25 * torch.exp(-EtaR * (distances - ShfR)**2)
        ret = rad * fc

        self.constant = Rcr
        self.save_for_backward(EtaR, ShfR, distances, fc, rad)
        return ret.flatten(start_dim=1)

    # @snoop
    def backward(self, grad_output):
        EtaR, ShfR, distances, fc, rad = self.saved_tensors
        Rcr = self.constant
        grad_output = grad_output.reshape(-1, 1, grad_output.shape[-1])

        fc_grad = (-0.5 * (math.pi / Rcr) * torch.sin(distances * (math.pi / Rcr)))
        rad_grad = -EtaR * 0.25 * (-2 * ShfR + 2 * distances) * torch.exp(-EtaR * (distances - ShfR)**2)

        # d(fg) / dx = (df/dx) * g + f * (dg/dx)
        dist_grad = grad_output * (rad_grad * fc + rad * fc_grad)
        return None, None, None, dist_grad.view(-1, grad_output.shape[-1]).sum(dim=-1)
...
class MyAngular_terms(MyAutograd):
    # @snoop
    def forward(self, Rca, ShfZ, EtaA, Zeta, ShfA, vectors12):
        ...
        fcj12 = 0.5 * torch.cos(distances12 * (math.pi / Rca)) + 0.5
        factor1 = ((1 + torch.cos(angles - ShfZ)) / 2) ** Zeta
        factor2 = torch.exp(-EtaA * (distances12.sum(0) / 2 - ShfA) ** 2)
        ret = 2 * factor1 * factor2 * fcj12.prod(0)
        self.constant = Rca
        self.save_for_backward(ShfZ, EtaA, Zeta, ShfA, vectors12, distances12, fcj12, factor1, factor2, angles)
    ...
    def backward(self, grad_output):
        # grad_output [42, 32]
        # angles [42, 1, 1, 1, 1]
        # vectors12 [2, 42, 3, 1, 1, 1, 1]
        # distances12 [2, 42, 1, 1, 1, 1]
        # fcj12 [2, 42, 1, 1, 1, 1]
        # factor1 [42, 1, 1, 1, 8]
        # factor2 [42, 1, 1, 4, 1]
        # ShfZ [1, 1, 1, 8]
        # ShfA [1, 1, 4, 1]
        ShfZ, EtaA, Zeta, ShfA, vectors12, distances12, fcj12, factor1, factor2, angles = self.saved_tensors
        Rca = self.constant
        c = 0.95
        grad_output = grad_output.view(1, grad_output.shape[0], 1, EtaA.shape[0], Zeta.shape[1], ShfA.shape[2], ShfZ.shape[3])  # [1, 42, 1, 1, 1, 4, 8]
        ShfZ = ShfZ.view(1, 1, 1, 1, 1, 1, -1)
        ShfA = ShfA.view(1, 1, 1, 1, 1, -1, 1)
        Zeta = Zeta.view(1, 1, 1, 1, -1, 1, 1)
        EtaA = EtaA.view(1, 1, 1, -1, 1, 1, 1)
        fcj12 = fcj12.unsqueeze(2)  # [2, 42, 1, 1, 1, 1, 1]
        distances12 = distances12.unsqueeze(2)  # [2, 42, 1, 1, 1, 1, 1]
        angles = angles.unsqueeze(0).unsqueeze(2)
        factor1 = factor1.unsqueeze(0).unsqueeze(2)
        factor2 = factor2.unsqueeze(0).unsqueeze(2)

        # [d(fc1*fc2)/dd1, d(fc1*fc2)/dd2] = [fc2*d(fc1)/dd1, fc1*d(fc2)/dd2]
        grad_fc_d12 = (-0.5 * (math.pi / Rca) * torch.sin(distances12 * (math.pi / Rca))) * fcj12.flip(0)  # [2, 42, 1, 1, 1, 1, 1]
        grad_factor1_angles = 2**(-Zeta) * Zeta * (torch.cos(ShfZ - angles) + 1)**(Zeta - 1) * torch.sin(ShfZ - angles)  # [1, 42, 1, 1, 1, 1, 8]
        grad_factor2_d12 = - EtaA * (distances12.sum(0, keepdim=True) / 2 - ShfA) * factor2  # [1, 42, 1, 1, 1, 4, 1]
        grad_d12_v12 = vectors12 / distances12  # [2, 42, 3, 1, 1, 1, 1]
        v12_dot = vectors12.prod(0, keepdim=True).sum(2, keepdim=True)  # [1, 42, 1, 1, 1, 1, 1]
        grad_angles_v12 = c * (v12_dot * vectors12 - distances12**2 * vectors12.flip(0)) / torch.sqrt(-(c * v12_dot * distances12**2)**2 + distances12.flip(0)**2 * distances12**6)  # [2, 42, 3, 1, 1, 1, 1] / [2, 42, 1, 1, 1, 1, 1]

        grad_fc_v12 = grad_fc_d12 * grad_d12_v12
        grad_factor1_v12 = grad_factor1_angles * grad_angles_v12
        grad_factor2_v12 = grad_factor2_d12 * grad_d12_v12

        grad_v12 = 2 * grad_output * (grad_factor1_v12 * factor2 * fcj12.prod(0, keepdim=True) + factor1 * grad_factor2_v12 * fcj12.prod(0, keepdim=True) + factor1 * factor2 * grad_fc_v12)  # [2, 42, 3, 1, 1, 4, 8]
        grad_v12 = grad_v12.flatten(start_dim=3).sum(-1)
        return None, None, None, None, None, grad_v12
...
class MyCompute_aev(MyAutograd):
    def __init__(self):
        self.r_op = MyRadial_terms()
        self.a_op = MyAngular_terms()

    # @snoop
    def forward(self, species, coordinates, triu_index, constants, sizes, cell_shifts):
        ...
        assert cell_shifts is None, "PBC is not supported for now"

        # PBC calculation is bypassed if there are no shifts
        if cell_shifts is None:
            atom_index12 = neighbor_pairs_nopbc(species == -1, coordinates, Rcr)
            selected_coordinates = coordinates_[atom_index12.view(-1)].view(2, -1, 3)
        ...
        radial_terms_ = self.r_op.forward(Rcr, EtaR, ShfR, distances)
        ...
        angular_terms_ = self.a_op.forward(Rca, ShfZ, EtaA, Zeta, ShfA, vec12)
        ...
        self.sizes = sizes
        self.save_for_backward(species, coordinates, index12, atom_index12_r, atom_index12, index, distances, vec_r, vec, pair_index12, sign12)
    ...
    # @snoop
    def backward(self, grad_output):
        species, coordinates, index12, atom_index12_r, atom_index12, index, distances, vec_r, vec, pair_index12, sign12 = self.saved_tensors
        Rcr, EtaR, ShfR, Rca, ShfZ, EtaA, Zeta, ShfA = self.constants
        num_species, radial_sublength, radial_length, angular_sublength, angular_length = self.sizes
        num_molecules = species.shape[0]
        coordinates_ = coordinates.flatten(0, 1)
        grad_coord = torch.zeros_like(coordinates_)  # [10, 3]
        grad_output_radial = grad_output[:, :, :radial_length]
        grad_output_angular = grad_output[:, :, radial_length:]

        # radial
        grad_output_radial = grad_output_radial.reshape(-1, radial_sublength)  # [2, 5, 64] -> [40, 16]
        grad_radial_terms_ = grad_output_radial[index12.flatten()].view(2, -1, radial_sublength).sum(0)  # [2, 16, 16]] -> [16, 16]]
        grad_dist_coord = vec_r / distances.unsqueeze(-1)  # [16, 3]
        grad_dist_coord = grad_dist_coord.unsqueeze(0) * torch.tensor([1, -1], device=grad_dist_coord.device).view(2, 1, 1)  # [2, 16, 3]
        _, _, _, grad_distances = self.r_op.backward(grad_radial_terms_)  # [16]
        grad_coord_ = grad_distances.unsqueeze(0).unsqueeze(-1) * grad_dist_coord  # [2, 16, 3]
        grad_coord.index_add_(0, atom_index12_r[0], grad_coord_[0])
        grad_coord.index_add_(0, atom_index12_r[1], grad_coord_[1])

        # angular
        grad_output_angular = grad_output_angular.reshape(-1, angular_sublength)  # [2, 5, 320] -> [40, 32]
        grad_angular_terms_ = grad_output_angular[index.flatten()]  # [42, 32]]
        _, _, _, _, _, grad_vec12 = self.a_op.backward(grad_angular_terms_)  # [2, 42, 3]
        grad_vec = torch.zeros_like(vec)  # [2, 16, 3]
        grad_vec.index_add_(0, pair_index12.view(-1), grad_vec12.view(-1, 3) * sign12.view(-1, 1))  # [2, 16, 3]
        grad_coord_ = grad_vec * torch.tensor([1, -1], device=grad_vec.device).view(2, 1, 1)  # [2, 16, 3]
        grad_coord.index_add_(0, atom_index12[0], grad_coord_[0])
        grad_coord.index_add_(0, atom_index12[1], grad_coord_[1])

        return None, grad_coord.reshape(num_molecules, -1, 3), None, None, None, None
...
def compute_aev(species: Tensor, coordinates: Tensor, triu_index: Tensor,
    ...
    # compute radial aev
    if return_radial_input:
        return (Rcr, EtaR, ShfR, distances)
    ...
    vec12 = vec.index_select(0, pair_index12.view(-1)).view(2, -1, 3) * sign12.unsqueeze(-1)
    species12_ = torch.where(sign12 == 1, species12_small[1], species12_small[0])
    if return_angular_input:
        return (Rca, ShfZ, EtaA, Zeta, ShfA, vec12)
...
class AEVComputer(torch.nn.Module):
    ...
    def forward(self, input_: Tuple[Tensor, Tensor],
        ...
        if cell is None and pbc is None:
            if return_computeaev_input:
                return species, coordinates, self.triu_index, self.constants(), self.sizes, None
