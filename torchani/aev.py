...
class SortedAEV(AEVComputer):
    ...
    def forward(self, coordinates, species):
        ...
        for s1, s2 in itertools.product(species_dedup, species_dedup):
            ...
            max_distances = distances.max(0)[0].max(0)[0]
            inRcr = (max_distances <= self.Rcr).nonzero().view(-1)
            inRca = (max_distances <= self.Rca).nonzero().view(-1)
            if s1 == s2:
            ...
            if torch.numel(inRca) > 0:
                ...
                vectors[(s1, s2)] = vec.gather(-2,
                                               indices.unsqueeze(-1).expand(*indices.shape, 3))

        del coordinate1, coordinate2, vec, distances, indices, max_distances, inRcr, inRca
