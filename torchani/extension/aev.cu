...
cub::CachingDeviceAllocator  g_allocator(2, 10, cub::CachingDeviceAllocator::INVALID_BIN, 1e9, false, false);
...
template<typename DataT>
__host__ __device__
bool operator==(const PairDist<DataT> & lhs, const PairDist<DataT> &rhs){
...
__global__
void
pairwiseDistance(torch::PackedTensorAccessor32<SpeciesT,2,torch::RestrictPtrTraits> species_t,
...
__global__
void
cuAngularAEVs(torch::PackedTensorAccessor32<SpeciesT,2,torch::RestrictPtrTraits> species_t,
	      torch::PackedTensorAccessor32<DataT,3,torch::RestrictPtrTraits> pos_t,
  ...
  for(int jj = 0; jj < jnum; jj++){
	...
	theta = acos(0.95 * (sdx[jj]*sdx[kk] +
			     sdy[jj]*sdy[kk] +
			     sdz[jj]*sdz[kk])/(Rij*Rik));
      }
      
      for(int srcLane = 0;
	  kk_start + srcLane < min(32, jnum);
	  ++srcLane){	
...
__global__
void
cuRadialAEVs(torch::PackedTensorAccessor32<SpeciesT,2,torch::RestrictPtrTraits> species_t,
...
template<typename DataT>
void
cubScan(const DataT *d_in, DataT *d_out, int num_items){
...
template<typename DataT,
	 typename IndexT>
int
cubEncode(const DataT *d_in, DataT *d_unique_out, IndexT *d_counts_out, int num_items, int *d_num_runs_out){
...
template<typename DataT,
	 typename LambdaOpT>
int
cubDeviceSelect(const DataT *d_in, DataT *d_out, int num_items, int *d_num_selected_out, LambdaOpT select_op){
  // Determine temporary device storage requirements
  void     *d_temp_storage = NULL;
  size_t   temp_storage_bytes = 0;
...
template<typename DataT>
DataT
cubMax(const DataT *d_in, int num_items, DataT *d_out){
  // Determine temporary device storage requirements
  void     *d_temp_storage = NULL;
  size_t   temp_storage_bytes = 0;
...
void
initConsts(AEVScalarParams<float> &aev_params){  
  ...
  cudaMemcpyToSymbol(csubaev_offsets, subaev_offsets,  sizeof(int) * num_species * num_species);
...
void
cuComputeAEV(torch::Tensor coordinates_t,
  ...
  //std::cout << "Running cuComputeAEV with " << n_molecules << "x" << max_natoms_per_mol << "\n";
  ...
  if(EtaR_t.size(0) != 1 || EtaA_t.size(0) != 1 || Zeta_t.size(0) != 1){
    std::cerr << "cuda extension is currently not supported for the specified configuration\n";
    exit(1);
  }
  ...
  CubDebugExit(g_allocator.DeviceAllocate((void**)&d_Rij, sizeof(PairDist<float>) * total_natom_pairs));
  ...
  // buffer to store all the pairwise distance that is needed for Radial AEV computation
  PairDist<float> *d_radialRij = 0;
  CubDebugExit(g_allocator.DeviceAllocate((void**)&d_radialRij, sizeof(PairDist<float>) * total_natom_pairs));
  //cudaMalloc((void **)&d_radialRij, sizeof(PairDist<float>) * total_natom_pairs);
  ...
  if((species_t.dtype() == torch::kInt32) &&
    ...
    pairwiseDistance 
      <<<n_molecules, block, sizeof(float) * max_natoms_per_mol * 3>>>
    ...
    int nRadialRij = cubDeviceSelect(d_Rij, d_radialRij, total_natom_pairs, d_count_out,
				     [=]__device__(const PairDist<float> d){
				       return d.Rij <= Rcr;
				     });

    int nblocks = (nRadialRij * 8 + block_size - 1)/block_size;
    cuRadialAEVs<int, float, 8>
      <<<nblocks, block_size>>>
      (species_t.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
    ...
    // Extract Rijs that is needed for AngularAEV comptuation i.e. all the Rij <= Rca
    int nAngularRij = cubDeviceSelect(d_radialRij, d_angularRij, nRadialRij, d_count_out,
				      [=]__device__(const PairDist<float> d){
					return d.Rij <= Rca;
				      });

    PairDist<float> *d_centralAtom = 0;
    CubDebugExit(g_allocator.DeviceAllocate((void**)&d_centralAtom, sizeof(PairDist<float>) * nAngularRij));
    //cudaMalloc((void **)&d_centralAtom, sizeof(PairDist<float>) * nAngularRij);
    
    
    int *d_numPairsPerCenterAtom = 0;
    CubDebugExit(g_allocator.DeviceAllocate((void **)&d_numPairsPerCenterAtom, sizeof(int) * nAngularRij));
    //cudaMalloc((void **)&d_numPairsPerCenterAtom, sizeof(int) * nAngularRij);
    
    // group by center atom
    int ncenter_atoms = cubEncode(d_angularRij, d_centralAtom, d_numPairsPerCenterAtom, nAngularRij, d_count_out);

    int *d_centerAtomStartIdx = 0;
    CubDebugExit(g_allocator.DeviceAllocate((void **)&d_centerAtomStartIdx, sizeof(int) * ncenter_atoms));
	...
	return (sm_aev + sxyz + sRij + sfc + sj) * ncatom_per_tpb;
      ...
      int maxNbrsPerCenterAtom = cubMax(d_numPairsPerCenterAtom, ncenter_atoms, d_count_out);

      int maxnbrs_per_atom_aligned = align<4>(maxNbrsPerCenterAtom);

      cuAngularAEVs
	<<<nblocks_angAEV, block_size, smem_size(maxnbrs_per_atom_aligned, block_size/nthreads_per_catom) >>>
	(species_t.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
	 coordinates_t.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
