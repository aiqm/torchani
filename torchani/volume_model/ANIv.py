...
class ANIv(torch.nn.Module):
    ...
    def forward(self, atomic_nums, coords, ensemble_values=False):
        ...
        neighbors = self.neighborlist(
            self.aev_computer.radial.cutoff, elem_idxs, coords, cell=None, pbc=None)
        aevs = self.aev_computer.compute_from_neighbors(
            elem_idxs, coords, neighbors)  # (BxNxD)
        volumes = coords.new_zeros(elem_idxs.shape)
        volumes += self.volume_networks(elem_idxs, aevs,
                                        atomic=True, ensemble_values=False)  # (BxN)
        volumes = torch.nn.Softplus(beta=10.0)(volumes)
