...
class TestGeometry(TestCase):
    def testCenterToComFrame(self):
        ...
        species, displaced_coordinates = torchani.geometry.displace_to_com_frame(
            (species, coordinates)
        )
        self.assertEqual(
            displaced_coordinates,
            coordinates - torch.tensor([[0.5, 0.5, 0.5]]).unsqueeze(1),
        )
    ...
    def testCenterToComFrameMany(self):
        species = torch.tensor(
            [[1, 1, 1, 1, 6, -1], [6, 6, 6, 6, 8, -1]], dtype=torch.long
        )
        coordinates = torch.tensor(
            [
                [
