...
if not MNP_IS_INSTALLED:
    ...
    raise unittest.SkipTest("MNP extension is not available, skipping all MNP tests.")
...
class TestInferMNP(ANITest):
    ...
    def _build_ani2x(
        self, idx: tp.Optional[int] = None, mnp: bool = False, infer: bool = True
    ):
    ...
    def testBmmEnsemble(self):
        self._test(
            self._build_ani2x(idx=None, infer=False),
            self._build_ani2x(mnp=False, idx=None, infer=True),
        )
    ...
    def testANIInfer(self):
        ...
        self._test(
            self._build_ani2x(idx=0, infer=False),
            self._build_ani2x(mnp=False, idx=0, infer=True),
        )
